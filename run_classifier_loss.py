...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  with tf.variable_scope("loss"):
    ...
    cond = tf.equal(sigmoid, tf.reduce_max(sigmoid))
    sigmoid_max = tf.where(cond, sigmoid, tf.zeros_like(sigmoid))
    is_correct = tf.reduce_sum(one_hot_labels * sigmoid_max, axis=-1)
